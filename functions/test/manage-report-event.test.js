...
test("generates daily AM_START report events at midnight without duplicates", () => {
  const now = new Date("2026-07-07T00:00:00.000Z");
  const events = generateDailyReportEventDocuments({
    schedules: [
      schedule(),
      schedule({ id: "worker-2_AM_START", workerId: "worker-2" })
    ],
    existingEvents: [event()],
    workerSettingsByWorkerId: {
      "worker-1": workerSettings(),
      "worker-2": workerSettings({
        userId: "worker-2",
        defaultEmploymentContext: "general_employment"
      })
    },
  ...
  assert.deepEqual(events.map((created) => created.id), [
    "worker-2_AM_START_2026-07-07T09:00:00.000Z"
  ]);
  assert.equal(events[0].workerId, "worker-2");
  assert.equal(events[0].dueAt.toISOString(), "2026-07-07T09:00:00.000Z");
  assert.equal(events[0].employmentContext, "general_employment");
  assert.equal(events[0].status, "pending");
});

test("skips disabled, non-AM_START, non-weekday, and missing-settings schedules", () => {
  const events = generateDailyReportEventDocuments({
    schedules: [
      schedule({ enabled: false }),
      schedule({ id: "worker-1_AM_END", type: "AM_END" }),
...
test("sends due AM_START push reminders with safe body and report click route", async () => {
  const sentPushes = [];
